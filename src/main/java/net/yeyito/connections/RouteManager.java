...
public class RouteManager {
    public static void addRoute(String host, String gateway, Integer netInterface) {
        try {
            ...
            Main.discordBot.sendMessageOnRegisteredChannel("all-item-price-changes",eCMD.toString() + " retrying in " + Main.getDefaultRetryTime() + " millis!",0);
    ...
    public static void deleteRoute(String host) {
        try {
            ...
            Main.discordBot.sendMessageOnRegisteredChannel("all-item-price-changes",e.toString() + " command to delete LIB route couldn't be processed, retrying in " + Main.getDefaultRetryTime() + " millis!",0);
            Main.threadSleep(Main.getDefaultRetryTime());
            deleteRoute(host);
        }
    }
